  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-[11px] text-zinc-400 font-medium">Timeline</span>
      </div>

      <div className="relative pl-3 space-y-4 before:absolute before:left-1.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-zinc-800">
        {events.map((ev, idx) => {
          const Icon = ev.icon;
          return (
            <div key={idx} className="relative flex items-start gap-3 pl-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-zinc-400 mt-0.5">
                  <span className="text-indigo-400 font-semibold">{ev.date}</span> · {ev.subtitle}
                </p>
              </div>
            </div>
          );
