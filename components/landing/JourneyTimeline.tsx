...
export function JourneyTimeline() {
  const milestones = [
    ...
    },
    {
      ...
      icon: Award,
      color: "text-[#E0E7FF]",
      bgColor: "bg-gradient-to-br from-[#4F46E5] to-[#22C55E]",
  ...
  return (
    <section className="py-20 bg-[#09090B] border-t border-[#27272A]/50 relative overflow-hidden">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        {/* Vertical Timeline Container (Mobile First 390px Stack) */}
        <div className="relative pl-6 sm:pl-10 space-y-6 sm:space-y-8 before:absolute before:left-[15px] sm:before:left-[23px] before:top-3 before:bottom-3 before:w-0.5 before:bg-gradient-to-b before:from-[#4F46E5] before:via-[#F59E0B] before:to-[#22C55E]">
          {milestones.map((item, idx) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`absolute -left-[31px] sm:-left-[43px] top-1 h-8 w-8 sm:h-10 sm:w-10 rounded-full ${item.bgColor} border-2 ${item.borderColor} flex items-center justify-center shadow-[0_0_15px_rgba(79,70,229,0.3)] z-10`}
                >
                  <IconComponent className={`h-4 w-4 sm:h-5 sm:w-5 ${item.day === "DAY 60" ? "text-white" : item.color}`} />
                </div>

                {/* Content Card */}
                <Card className="bg-[#18181B] border-[#27272A] p-5 sm:p-6 w-full hover:border-[#3F3F46] transition-all">
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-bold text-[#818CF8] px-2 py-0.5 rounded bg-[#4F46E5]/10 border border-[#4F46E5]/30">
                        {item.day}
                      </span>
                  ...
                  <p className="text-xs sm:text-sm text-[#A1A1AA] leading-relaxed mt-1">
                    {item.description}
                  </p>
                </Card>
              </motion.div>
